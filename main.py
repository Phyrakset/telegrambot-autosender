...
def main():
    parser = argparse.ArgumentParser(
        description="TeleSender Pro - Telegram Lead Outreach, Workingna Candidate Query & TverKar Migration Suite",
        epilog="Examples:\n  python main.py --ui\n  python main.py send --db --db-limit 50\n  python main.py send phone-list.txt --delay 2",
    ...
    # Command: send
    parser_send = subparsers.add_parser("send", help="Auto-send TverKar video message, survey & prefill migration into TverKar (CLI)")
    parser_send.add_argument("file", nargs="?", default="phone-list.txt", help="Phone list path (if not using --db)")
    parser_send.add_argument("--db", action="store_true", help="Fetch candidate phone numbers directly from Workingna MySQL DB")
    parser_send.add_argument("--db-limit", type=int, default=50, help="Max candidates to load from Workingna DB")
    parser_send.add_argument("--db-looking", action="store_true", help="Filter for candidates with lookingForJobs=1 only")
    parser_send.add_argument("--search", default=None, help="Filter candidate search keyword in Workingna DB")
